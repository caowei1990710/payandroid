...
public class QHBNotificationService extends NotificationListenerService {
    ...
    public void onNotificationPosted(final StatusBarNotification sbn) {
        ...
        if (sbn.getPackageName().equals(WechatAccessbilityJob.WECHAT_PACKAGENAME)) {
            Log.e(TAG, sbn.getNotification().tickerText + "");
            if(sbn.getNotification().tickerText.toString().indexOf("微信支付:")!=-1){
                Intent intent = new Intent(Config.BEGINTASK);
                service.getBaseContext().sendBroadcast(intent);
            }
        }else if(sbn.getPackageName().equals("com.eg.android.AlipayGphone")){
            PendingIntent pendingIntent = sbn.getNotification().contentIntent;
            boolean lock = NotifyHelper.isLockScreen(service.getBaseContext());
            Intent intent = new Intent(Config.NOTIFYINSERT);
            service.getBaseContext().sendBroadcast(intent);
